 ...
 * @Description: 
 * @Autor: TJUZQC
 * @Date: 2020-07-02 00:22:45
 * @LastEditors: TJUZQC
 * @LastEditTime: 2020-07-02 00:28:27
-->
<!DOCTYPE html>
<html>
<head>
        ...
        function getLocation() {
            ...
            map.plugin("AMap.Geolocation", function () {
                ...
                function onComplete(data) {
                    $("#cityname").val(data.addressComponent.city);
                    console.log("located!");
                    getWeather();
                }
        ...
        function getWeather() {
            ...
            $.post(
                ...
                function (data, status) {
                    var live = JSON.parse(data.live.substr(1, data.live.length - 2));
                    var cast = JSON.parse(data.cast.substr(1, data.cast.length - 2));
                    if (status == "success" && live.status == "1" && cast.status == "1") {
                        live = live.lives[0];
                        ...
                        for (day in cast) {
                            $("#casts_body").append(
                                "<tr>" +
                                "<td id=\"date\">" + cast[day].date + " 星期" + cast[day].week + "</td>" +
                                "<td id=\"weather\">"+(cast[day].dayweather==cast[day].nightweather?cast[day].dayweather:(cast[day].dayweather+"转"+cast[day].nightweather))+"</td>" +
                                "<td id=\"temp\">"+Math.min(cast[day].daytemp,cast[day].nighttemp)+"℃"+"-"+Math.max(cast[day].daytemp,cast[day].nighttemp)+"℃"+"</td>" +
                                "<td id=\"wind\">"+(cast[day].daywind==cast[day].nightwind?(cast[day].daywind+"风"):(cast[day].daywind+"风转"+cast[day].nightweather+"风"))+"</td>" +
                                "<td id=\"power\">"+(cast[day].daypower==cast[day].nightpower?(cast[day].daypower+"级"):(cast[day].daypower+"级转"+cast[day].nightpower+"级"))+"</td>" +
                                "</tr>");
                        }
                    } else {
...
<div class="head_nav_C" id="hongye_nav" style="text-align: center;margin-top: 20px">
